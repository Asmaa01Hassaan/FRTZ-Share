...
class Projecttask(models.Model):
    ...
    def task_timesheet_reminder(self):
        current_user = self.env['res.users'].browse(request.env.uid)
            ...
            if task.total_hours_spent > task.allocated_hours:
                ...
                if template_id:
                    ...
                    for res_id, val in list(values.items()):
                        val['email_from'] = current_user.email
                        val['res_id'] = False
                        val['author_id'] = current_user.partner_id.id
    ...
    def start_task_button(self):
        ...
        allow_multi_task = self.env['ir.config_parameter'].sudo().get_param(
